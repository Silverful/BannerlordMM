    ...
    public class PlayerStatItemResponse
    {
        ...
        public static PlayerStatItemResponse FromMatchRecordGrouping(Player player, IGrouping<Guid, PlayerMatchRecord> record, IDictionary<string, double> RankTable)
        {
            return new PlayerStatItemResponse
            {
                ...
                KR = record == null ? 0 : ((double)record?.Sum(x => x.Kills)).SafeDivisionWithInfinity((double)record?.Sum(x => x.Match.RoundsPlayed)),
                ...
                KD = record == null ? 0 : ((double)record?.Sum(x => x.Kills)).SafeDivisionWithInfinity((double)record?.Sum(x => x.Deaths)),
                KDA = record == null ? 0 : ((double)record?.Sum(x => x.Kills) + (double)record?.Sum(x => x.Assists)).SafeDivisionWithInfinity((double)record?.Sum(x => x.Deaths)),
                DR = record == null ? 0 : ((double)record?.Sum(x => x.Deaths)).SafeDivisionWithInfinity((double)record?.Sum(x => x.Match.RoundsPlayed)),
                Score = record?.Sum(x => x.Score) ?? 0,
                SR = record == null ? 0 : ((double)record?.Sum(x => x.Score)).SafeDivisionWithInfinity((double)record?.Sum(x => x.Match.RoundsPlayed)),
                MVP = record?.Sum(x => x.MVPs) ?? 0,
                MVPR = record == null? 0 : ((double)record?.Sum(x => x.MVPs)).SafeDivisionWithInfinity((double)record?.Sum(x => x.Match.RoundsPlayed))
            };
        }
    }
}
